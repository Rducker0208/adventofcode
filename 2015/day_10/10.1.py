...
for i in range(40):
    new_string = ''
    parts = [''.join(g) for _, g in groupby(string)]

    for part in parts:
        new_string += str(len(part))
        new_string += part[0]
...
for i in range(40):
    ...
    element_string = ''.join([sequence_dict[element] for element in new_elements])
    if element_string != string_dict[i]:
