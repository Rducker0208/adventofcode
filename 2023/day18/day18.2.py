...
def extract_steps(hex_code):
    new_direction = directions[hex_code[-1]]
    steps = int(hex_code[:-1], 16)
    return new_direction, steps


for rule in rules:
    hex_string = rule.split()[2][2:-1]
    direction, step_amount = extract_steps(hex_string)
